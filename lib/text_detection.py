...
class TextDetection(object):
    ...
    def __init__(self, image_file, config, direction='both+', details=False):
    ...
    def detect(self):
        #retrun array of given image
        res9 = np.zeros_like(self.img)

        #stroring copy of res9 into res0 to res8
        if self.details:
        ...
        cv2.polylines(self.final2, hulls, 1, (0, 255, 0))
        ...
        for i, (region, bbox) in enumerate(zip(regions, bboxes)):
            ...
            #-----------------
            #stroke is width of line or edge
        ...
        if self.details:
            plt_show((self.img, "Image"), \
                     ...
                     (res6, "STROKES TOTAL COUNT={},({} regions)".format(self.SWT_TOTAL_COUNT, n6)),\
